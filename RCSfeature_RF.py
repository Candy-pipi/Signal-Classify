from RCSfeature_ML_prepare import read_data
from sklearn.ensemble import RandomForestClassifier
from sklearn.metrics import confusion_matrix
import numpy as np


signal_file = r'RCSsequence_feature'
label_file = r'RCS_label'
num_classes = 8  # 类别数
ntrain = 5  # 选择每类训练样本数量
sampling_point = 10   # RCS为2000，一维距离像为512,特征数RCS为10，HRRP按算法设置

# 数据准备
(x_train, y_train), (x_test, y_test) = read_data(signal_file, label_file, ntrain_per_class=ntrain)

# 模型搭建及训练
clf = RandomForestClassifier(n_estimators=200, max_features=10, oob_score=True)
# 可调参数决策树个数n_estimetors，每棵树最大特征数
